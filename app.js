    ...
    const unProgramador = await Programador.findByPk(id, {
      include: [
      {
        ...
        through: {
          ...
          attributes: {exclude: ['createdAt','updatedAt', 'activo', 'descripcion','programador_proyecto']}
      },
      ]
    });
  
    if (unProgramador === null) {
      res.status(404).json({ error: `No se encontró al Programador con ID ${id}.` });
    } else {
...
app.patch('/programadores/:id', async (req, res) => {
  ...
  try {
    const [, affectedRows] = await Programador.update(unProgramador, { where: { id } });
    if (affectedRows === 0) {
...
app.get('/proyectos', async (req, res) => {
  ...
  attributes: {exclude: ['descripcion','lenguaje']}
});
  res.json(data);
});
    ...
    const unProyecto = await Proyecto.findByPk(id,{
      include: [
      {
        ...
        through: {
          ...
          attributes: {exclude: ['programador_proyecto', 'email','createdAt','updatedAt']}
      },
      ]
    });
...
app.patch('/proyectos/:id', async (req, res) => {
  ...
  try {
    const [, affectedRows] = await Proyecto.update(unProyecto, { where: { id } });
    if (affectedRows === 0) {
      res.status(404).json({ error: `No se encontró el Proyecto con ID ${id}.` });
    } else {
      res.json({ id: id });
...
app.post('/proyectos/:idproyecto/programadores/:idprogramador', async (req, res) => {
  ...
  try {
    ...
    let proyectos = await Proyecto.findByPk(idproyecto)
    let programadores = await Programador.findByPk(idprogramador)

    await proyectos.addProgramadores(programadores)
...
app.delete('/proyectos/:idproyecto/programadores/:idprogramador', async (req, res) => {
  ...
  try {
    ...
    await proyectos.removeProgramadores(programadores)
